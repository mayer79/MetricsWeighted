...
- `accuracy()`, `recall()`, `precision()`, `f1_score()`, and `classification_error()`: Typical binary performance measures derived from the confusion matrix, see (https://en.wikipedia.org/wiki/Precision_and_recall). Require binary predictions. Except for classification error, higher values are better.
...
- `deviance_bernoulli()` and `logLoss()`: Further metrics relevant for binary targets, namely the average unit deviance of the *binary* logistic regression model (0-1 response) and log loss (half that deviance). As with all deviance measures, smaller values are better.
...
In order to facilitate the use of these metrics in a {dplyr} chain, use the function `performance()`: Starting from a data set with actual and predicted values (and optional case weights), it calculates one or more metrics. The resulting values are returned as a `data.frame`. 
