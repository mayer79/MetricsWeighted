...
- `deviance_tweedie`: Average unit Tweedie deviance with parameter `tweedie_p`.
...
Notable exception is `deviance_tweedie` that requires the Tweedie power as additional parameter. The value 0 corresponds to the Gaussian/normal deviance, the value 1 to the Poisson deviance and the value 2 to the Gamma deviance. For fixed actual and predicted values, the Tweedie deviance is continuous with respect to this parameter.
...
Furthermore, we provide a very general function `r_squared`, defined as proportion of deviance explained, i.e. 1 minus the ratio of residual deviance and intercept-only deviance. By default, it calculates the ordinary R-squared, i.e. proportion of *normal* deviance explained. However, you can specify any different deviance function, e.g. `deviance_tweedie` with paramter 1.5 or the deviance of the logistic regression (`deviance_bernoulli`).
